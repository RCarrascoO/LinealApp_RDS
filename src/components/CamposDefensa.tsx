  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className={`w-full rounded-lg border px-3 py-2 text-sm outline-none transition-colors placeholder:text-muted-foreground focus:ring-2 focus:ring-ring/30 ${borde}`}
      />
      {status && (
        <p className={`mt-1 text-xs font-medium ${status.correcto ? 'text-success' : 'text-destructive'}`}>
  ...
  return (
    <section className="rounded-2xl border border-border bg-card p-6 shadow-sm">
      <div className="mb-5 flex items-start justify-between gap-4">
        <div>
          <h2 className="text-2xl font-semibold text-foreground">{tituloSeccion}</h2>
      ...
      {verificado && (
        <div className="mt-6 rounded-xl border border-border bg-muted p-4 text-sm text-foreground">
